#!/bin/bash
# Differential Testing - AST Comparison
# Compares AST outputs between aster0 and aster1
...
# Print header
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  Differential Testing - AST Comparison"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
test_file() {
    ...
    # Generate AST with aster1
    local test_output=$(mktemp)
    if ! "$ASTER1" emit-ast-json "$fixture" > "$test_output" 2>/dev/null; then
        echo -e "  ${RED}✗${NC} $filename: aster1 failed to emit AST"
        total_failed=$((total_failed + 1))
        rm -f "$test_output"
        return 1
    fi
...
for category in "${CATEGORIES[@]}"; do
    ...
    goldens_path="$GOLDENS_DIR/$category/ast"
...
if [ "$ASTER1_EXISTS" = false ]; then
    if [ $total_failed -eq 0 ]; then
        echo -e "${GREEN}All AST golden files verified!${NC}"
    ...
    if [ $total_failed -eq 0 ]; then
        echo -e "${GREEN}All differential AST tests passed!${NC}"
        echo "aster0 and aster1 produce identical AST outputs."
    else
        echo -e "${RED}$total_failed/$total_tests differential AST tests failed.${NC}"
